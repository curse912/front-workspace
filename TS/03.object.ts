...
// 선언 병합시 주의점
// - 기존 인터페이스가 가지고 잇는 속성과 중복된 속성을 사용하는 경우 
//   반드시 타입을 일치 시켜줘야한다.
interface Bird{
    name:string;
    // wing:boolean;   // 기존 속성의 타입 변경 불가
}
...
// const bugger:Food2 = {name:'맥도날드'};
// const flavor:Food2 = {flavor:"짠맛"};
...
/*
    4. 결론
     ...
     - 단, 타입작성 목적 및 내뷰 구조의 구현 방식에 따라 적합한 설계방식이 나뉘게 된다.
     
     1) interface가 적합한 경우
      - 클래스와 함께 사용시, 구조 호가장과 유지보수가 필요한 경우
      - 디버깅시 타입표시가 필요한 경우
...
// 인덱스 시그니쳐 사용시 주의점
// - 속성에 여러값들이 들어가는 경우, 인덱스 시그니쳐는 그 모든 값들에 대해 표현할 수 있어야한다.
...
/*
    6. 타입호환성
     ...
     - 타입 스크립트는 호환성 여부를 체크할때 '구조적 타이핑'에 기반하여 호환가능 여부를 판단한다.
     - 구조란 객체의 '속성명'과 그 '타입'을 의미
     - 구조적 타이핑의 메커니즘으로 인해 객체으이 모든 속성이 완전히 일치하지 않더라도 타입 호환이 가능하다.
     - 모든 타입이 완전히 일치하지 않더라도 타입 호환이 가능하다.
*/
type Teacher = {
    name:string,
    age:number
...
const eunsung:Student = sim;    // location속성은 student에 없지만 나머지 구조가 일치하기 때문에 호환된다.
