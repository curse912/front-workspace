/*
    1. Class
     - javascript의 class문법을 보다 확장하여, 좀 더 객체 지향적인 프로그래밍이 가능하도록 만들었다.
     - 접근 제한자, 추상클래스, 인터페이스 구현등의 기능을 제공
*/
...
// - class 생성시의 반복되는 코드를 줄일 수 있는 단축 문법
class Product{
    //필드 선언 생략, 초기화 코드 생략
...
new Product(1,'로봇청소기',7000,'samsung').printInfo();

/*
    3. Generics
     - Type을 변수처럼 외부에서 주입받아 사용하는 기능으로 타입변수라고 불린다.
     - Type에 유연성을 제공하면서 확장성과 타입안정성을 추가 할 수 있다.
     * Generics의 주요기능
      - extends키워드를 통한 상한 경계 설정(super는 지원하지 않음)
      - Default Type
*/
...
*/
// Animal을 상속 받았거나 하위 타입만 받는 함수를 작성
function func<T extends Animal>(animal:T){
    animal.bark();
}

// TypeScript는 구조 중심의 타입 검사 시스템을 사용하기 때문에 다음과 같이 작성가능
...
// 3) 제네릭의 기본 타입
// - 제네릭 사용되지 않는 경우 타입변수에 대입될 기본값 지정
...
function getValue<K extends keyof Person2>(person:Person2 , key:K): Person2[K]{
    return person[key];
}

// function getValue<P extends keyof Person2,K extends KeyOfPerson>(person:P, key:K): P[K]{
//     return person[key];
// }
...
/*
    4. Utility Type
     - 기존 객체 타입들을 변형 및 조작할 수 있도록 도와주는 유틸리티 타입들

     1) Partial<T>
      - T의 모든 속성을 oprional속성으로 변경해주는 유틸리티 타입
...
// const user:User = {id:1,name:'aaa'};    //수정할 데이터 중 email이 없는 경우 에러
const PartialUser:Partial<User> = {id:1, name:'aa'};
...
// 3) Omit<T,K>
// - 객체타입에서 특정 속성을 제외하고 나머지만 남긴 새 타입을 만들때 사용
...
/*
    Utility Type 조합
     ...
     같은 속성이 겹치는 경우 더 구체적인 속성으로 우선시 되어 병합
...
/*
    5) ReturnType<T>
     - 함수 타입 T가 반환하는 결과의 타입을 그대로 가져오는 유틸리티 타입
*/
function getUser(){
