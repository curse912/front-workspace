...
export default function MenuInput(){
    ...
    /*
        #2. useMutation
         ...
         - 요청을 보낸 후 요청상태(loading, error, success)를 관리할 수 있으며,
           요청 성공시 캐시 데이터를 무효화(incalidate)시켜 관련 데이터를 refetch한다.

        [사용법]
        const queryClient = useQueryClient();   //전역 캐시관리 객체(쿼리무효화에 사용한다.)
        ...
        <>
            <div className="menu-test">
                ...
                <form id="menuEnrollFrm" onSubmit={insertMenu} >
                    ...
                    <br />
                    <input type="submit" className="btn btn-block btn-outline-success btn-send" value="등록" disabled={mutation.isPending} /*무의미한 재요청 막기*//>
                </form>
            </div>
        </>
    )
