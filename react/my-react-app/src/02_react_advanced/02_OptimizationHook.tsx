...
/*
    최적화를 위해 사용하는 훅 함수들 
     - useMemo,useCallback
     - 컴포넡트의 리렌더링을 줄여 성능을 최적화 해준다.
*/
function OptimizationHook(){

    /*
        #1. useMemo
         - 계산결과를 메모해두는 훅
         - 리렌더링 시 계산비용이 큰 작업의 결과를 메모이제션(캐싱)하여, 렌더링을 최적화한다.
   ...
   const heavyComputation = (list: number[]) => {
    ...
    // 컴포넌트가 랜더링 될때마다 정렬된 배열을 다시 계산하는 중.
    // const sorted = heavyComputation(numbers);

    //useMemo를 활용해 배열을 캐싱
    // 현재 컴포넌트가 랜더링 될때마다 정렬된 배열을  다시 계산하는 것이 아니라
    ...
    /*
        #2. useCallback
         - 함수를 메모해두는 훅
         - 매 렌더링 마다 새롭게 만들어지는 함수 생성 비용을 줄여준다.
         - 함수를 부모 컴포넌트에서 생성하여 자식 컴포넌트로 전달하는 경우, 함수가 새롭게 생성되면서
         자식컴포넌트 또한 리 렌더링 되는데 이를 방지하기 위해 사용
         - React.memo
    */
...
const ChildList = React.memo(({onClick}:{onClick:()=>void}) =>{
    ...
    return <button onClick={onClick}>정렬 결과보기</button>

})

export default OptimizationHook;
