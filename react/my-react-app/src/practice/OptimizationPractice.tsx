...
function OptimizationPractice() {
  ...
  // => return에 검색어 입력에서 ref = {inputRef}사용 : 원할때만 리랜더링
  // => showCount : setKeyword(inputRef.current?.value ?? "")  --> inputRef에 현재값이 있으면 밸류값, 없으면  "" 공란

  
  //  2) 현재filtered는 컴포넌트가 렌더링 될때 마다 필터링 연산이 실행된다.
  //    이를 최적화 하여 keyword가 변경되지 않는 한 불필요한 연산이 수행되지 않도록 최적화 하시오.
  // => filterd를 useMemo로 감싸줌.
  
  // 현재 계속 리렌더링 되고있음  ( 훅 함수 추가 필요 )
  const filtered = useMemo(()=>
    // if(!searchKey.trim()){
    //   return users;
    // }
    heavyFilter(users, keyword)
  ,[users,keyword]);
  ...
  return (
    <div>
      ...
      <input
        ...
        // value={keyword}
      />
      <p>검색 결과 ({filtered.length}명): {filtered.slice(0, 20).join(", ")}</p>
