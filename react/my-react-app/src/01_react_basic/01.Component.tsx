/*
    #1. 컴포넌트의 이름은 "대문자"로 시작한다.
     - 리엑트는 렌더링 할 요소가 소문자로 시작하는 경우 html 태그로, 
       대문자로 시작하는 태그는 리액트 컴포넌트로 인식
*/
function Component(){

    /*
        #2. 함수형. 컴포넌트는 반드시 1개의 jsx요소를 반환해야한다.
         - 여러태그를 동시에 반환하고자 한다면 <></>로 감싸준다.
    ...
    return(
        /*
            #4. 모든 요소는 반드시 "닫아"줘야한다.
             - <img>,<input>등 종료 태그가 존재하지 않는 태그도, 닫아줘야한다.
            #5. jsx문법에서 자바 스크립트 변수를 사용시 {}를 이용한다.
...
function Footer(){
    ...
    */
    const clickHandler = function(){
        console.log("클릭해주셔서 감사합니다🙇‍♀️")
    }
    return(
        // #7. 클래스 속성은 className으로 작성한다.
        //   - jsx문법 내부에서 class 는 class생성 예약어를 의미.
